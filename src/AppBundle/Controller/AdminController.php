...
class AdminController extends Controller
{
    ...
    /**
      * @Route("/importOmdb/{id}", name="import_omdb")
      * @Template()
      */
    public function importOmdbAction($id)
    {
        $success = false;
        $omdb = new OMDbAPI();

        if ( $omdb->validateIMDBid($id) ) {
            $show = ($omdb->fetch('i', $id))->data;

            if ($show->Type == 'series') {
                $showToSave = new TVShow;
                $showToSave->setName($show->Title);
                $showToSave->setSynopsis($show->Plot);

                if ($show->Poster) {
                    $ext = explode(".", $show->Poster);
                    $ext = $ext[count($ext)-1];
                    ...
                    copy($show->Poster, $webRoot.'/uploads/'.$filename);
                    $showToSave->setImage($filename);
                }
                
                $em = $this->get('doctrine')->getManager();
                $em->persist($showToSave);
                $em->flush();
                $success = true;
            }
        }
        ...
        if ($success)
            return $this->redirect($this->generateUrl('show', ['id' => $showToSave->getId()]));
        else
